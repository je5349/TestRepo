...
bool LocalPlannerUtil::path_tracking_controller(double total_distance, int mode)
{
	...
	double max_angle_turn = 3.14159/6;
	//**************************************************************//
	flag = mode;
	...
	else 
	{
		...
		if (left < right)
		{
			right_turn = (2 - left)*0.64/2;
			diff_turn = -right_turn;
		}
		
		else if (right < left)
		{
			left_turn = (2 - right)*0.64/2;
			diff_turn = left_turn;
		}
		...
		//diff_turn = left_turn - right_turn;
		if (diff_turn > 0.64)
		{
			diff_turn = 0.64;
		}
		...
		local_path.push_back(pose.y + diff_turn);
	}
	...
	choose_path();
		...
		if(path2select == OFFSET_ORIGINAL_PATH)
		{
			...
			/*x1 = global_x + LR_x*cos(pose.heading) - LR_y*sin(pose.heading);
			y1 = global_y + LR_x*sin(pose.heading) + LR_y*cos(pose.heading);

			x1 = x1*cos(pose.heading) + y1*sin(pose.heading);
			y1 = -x1*sin(pose.heading) + y1*cos(pose.heading);

			x2 = global_x + LL_x*cos(pose.heading) - LL_y*sin(pose.heading);
			y2 = global_y + LL_x*sin(pose.heading) + LL_y*cos(pose.heading);
		...
		direction = atan2((end_point.y - start_point.y), (end_point.x - start_point.x));
		...
		ROS_INFO("turn angle: %f", diff_turn);
		...
		distance_travelled += hypot(global_x - prev_pose_x , global_y - prev_pose_y);
		ROS_INFO("Distance travelled = %f \n", distance_travelled);
	
		prev_pose_x = global_x;
		prev_pose_y = global_y;
...
void LocalPlannerUtil::choose_path()
{
	...
	double closest_localpath_trajectory_heading = atan2(local_path[1] - start_point.y, local_path[0] - start_point.x);
