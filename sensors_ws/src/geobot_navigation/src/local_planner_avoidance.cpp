...
#include <local_planner_avoidance.h>
...
LocalPlannerUtil::LocalPlannerUtil(): nhPriv("~"), move2goal_action_(nh, "move2goal",boost::bind(&LocalPlannerUtil::run, this, _1), false), f(boost::bind(&LocalPlannerUtil::parameter_callback, this, _1, _2))
{
	...
	//*********************Additional controller terms for PID***********************/
	nhPriv.param("proportional_gain",proportional_gain, 0.0);
	nhPriv.param("integral_gain",integral_gain, 0.0);
	nhPriv.param("proportional_heading_gain",proportional_heading_gain, 0.0);
	nhPriv.param("integral_heading_gain",integral_heading_gain, 0.0);
	//*******************************************************************************/
	...
	//make parameter
	updatedpath_filename = "/home/rahuldapp/ros_ws/src/geobot_gui/src/real_path.txt";
	utmZone = "13S";
...
void LocalPlannerUtil::parameter_callback(geobot_navigation::GainsConfig &config, uint32_t level)
{
	...
	minimum_velocity_constraint = config.minimum_velocity_constraint;           
...
//		cout << "diff_angle-----"<< diff_angle << endl;
	...
	cout << "lookahead_heading_deviation-----"<< lookahead_heading_deviation << endl;
...
//	  std::ofstream fileOUT;
  ...
  //    fileOUT.open(updatedpath_filename.c_str(), std::ofstream::out | std::ofstream::trunc);
...
/*
				...
				*/
				...
				if(gap_position2obstacle == GAP_RIGHT_SIDE_OF_OBSTACLE)
				{
					...
					ROS_INFO("offset to right %f", path_offset);
				}
				else
				{
...
/*
				...
				*/
				...
				//ROS_INFO("x_center %f", x_center);
				//ROS_INFO("y_center %f", y_center);
							

				//ROS_INFO("new_traj_heading %f", (new_traj_heading/M_PI)*180);
...
//				cout << "bendahead-----"<< bendahead << " pose.heading-----"<< pose.heading << " pose.x-----"<< pose.x << " pose.y-----"<< pose.y << endl;
			...
			velocity_constraint = (1 - pow((bendahead/(3.14*0.45)),1.5));
			//velocity_constraint = exp(-(bendahead/(3.14*0.45))/0.3);	
//			cout << "velocity_constraint-----"<< velocity_constraint << endl;
			//velocity_constraint = max(velocity_constraint,minimum_velocity_constraint);
...
//			cout << "cross_track_error-----"<< cross_track_error << endl;
             		...
             		if(delta_time > ros::Duration(min_delta_time))
			{
				...
				//***********Adding all my extra controller terms here*******************//
				//Proportional controller//
				proportional_error_term = proportional_gain * cross_track_error;
				
				//Integral controller//
				integral_error += cross_track_error*delta_time.toSec();
				//Adding winding up terms
				if ( integral_error > fabsf(windup_limit))
    			integral_error = fabsf(windup_limit);
  				if ( integral_error < -fabsf(windup_limit))
    			integral_error = -fabsf(windup_limit);
    			//Final Integral Error term//
    			integral_error_term = integral_gain * integral_error;
    			
    			//Overall controller
    			controller_error_term = derivative_error_term + integral_error_term + proportional_error_term;
    			//***********************************************************************//
				...
				proportional_heading_error = proportional_heading_gain * error_heading;

				//Integral heading controller//
				integral_heading += error_heading*delta_time.toSec();
				//Adding winding up terms
				if ( integral_heading > fabsf(windup_limit))
    			integral_heading = fabsf(windup_limit);
  				if ( integral_heading < -fabsf(windup_limit))
    			integral_heading = -fabsf(windup_limit);

    			//Final Integral heading error term//
    			integral_heading_error = integral_heading_gain * integral_heading;
    			
    			//Overall heading controller
    			controller_heading_error_term = derivative_heading_error_term + proportional_heading_error + integral_heading_error;
    			//***********************************************************************//
...
//ROS_INFO("error difference component  %f", delta_cross_track_error);
			...
			steering_angle = heading_gain*error_heading + error_gain*atan2(cte_gain*cross_track_error,linear_velocity) + controller_error_term + controller_heading_error_term;  //check this
...
*/	
	...
	if(!candidate_gap_msg.gaps.empty()) // That is, if there are gaps to drive through
	{
		for(int i = 0; i < candidate_gap_msg.gaps.size(); i++)
		{
			...
			//ROS_INFO("x2 %f", x2);
			//ROS_INFO("y2 %f", y2);
			//ROS_INFO("SR_heading %f", SR_heading);
			//ROS_INFO("SR_diff %f", SR_diff);
			//ROS_INFO("SL_heading %f", SL_heading);
			//ROS_INFO("SL_diff %f", SL_diff);
	...
	ROS_INFO("which_gap_seq %d", which_gap_seq); 

	ROS_INFO("gap_position2obstacle %d", gap_position2obstacle); 
...
*/
	...
	if(gap_position2obstacle != NO_GAP)
	{
		...
		else if((obstacle_array[0] == LEFT_SIDE_OPENED)&& (obstacle_array[1] == FRONT_SIDE_BLOCKED) && (obstacle_array[2] == RIGHT_SIDE_BLOCKED))
		{
			...
			{path2select = OFFSET_ORIGINAL_PATH;}
			else if ((shortest_distance_to_obstacle_LS > (side_distance_avoidance_threshold + side_avoidance_blindband)) && (onwhich_sideofrobot_is_originalpath == LEFT_SIDE_OF_ROBOT))
			{path2select = ORIGINAL_PATH;}
		}
		else if((obstacle_array[0] == LEFT_SIDE_BLOCKED) && (obstacle_array[1] == FRONT_SIDE_BLOCKED) && (obstacle_array[2] == RIGHT_SIDE_OPENED))
		{
			...
			if(gap_position2obstacle == GAP_RIGHT_SIDE_OF_OBSTACLE)
			{ path2select = OFFSET_ORIGINAL_PATH;}
			else if ((shortest_distance_to_obstacle_RS > (side_distance_avoidance_threshold + side_avoidance_blindband)) && (onwhich_sideofrobot_is_originalpath == RIGHT_SIDE_OF_ROBOT))
			{path2select = ORIGINAL_PATH;}
		}

		/* if( path2select == OFFSET_ORIGINAL_PATH && (shortest_distance_to_obstacle_LS > (side_distance_avoidance_threshold + side_avoidance_blindband)) && (onwhich_sideofrobot_is_originalpath == LEFT_SIDE_OF_ROBOT))
		{
			path2select = ORIGINAL_PATH;
		}
		if( path2select == OFFSET_ORIGINAL_PATH && (shortest_distance_to_obstacle_RS > (side_distance_avoidance_threshold + side_avoidance_blindband)) && (onwhich_sideofrobot_is_originalpath == RIGHT_SIDE_OF_ROBOT))
		{
			path2select = ORIGINAL_PATH;
		}
		if((obstacle_array[0] == LEFT_SIDE_OPENED) && (obstacle_array[1] == FRONT_SIDE_BLOCKED) && (obstacle_array[2] == RIGHT_SIDE_OPENED))
		{
			path2select = OFFSET_ORIGINAL_PATH;
		}	*/
	...
	ROS_INFO("path2select %d", path2select); 
	ROS_INFO("onwhich_sideofrobot_is_originalpath %d", onwhich_sideofrobot_is_originalpath);
...
bool LocalPlannerUtil::run(const geobot_navigation::SetGoalGoalConstPtr& req)
{
