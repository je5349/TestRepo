...
bool LocalPlannerUtil::path_tracking_controller(double total_distance, int mode)
{
	...
	else
	{
		if(left < right)
		{
			local_path.push_back(pose.y - ((2-left)/8)); 
		}
		else if(right < left)
		{
			local_path.push_back(pose.y + ((2-right)/8)); 
		}
		else
		{
			local_path.push_back(pose.y);
	...
	choose_path();
		...
		else
		{
			...
			//compass_correction_gain = 0.1*(compass - current_compass;
		}
		...
		steering_angle = heading_gain*error_heading + error_gain*atan2(cte_gain*cross_track_error,linear_velocity) + controller_error_term + controller_heading_error_term - compass_correction_gain;
