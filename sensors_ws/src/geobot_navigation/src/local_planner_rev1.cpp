...
double LocalPlannerUtil::compute_along_track_error()
{
	 ...
	 L1 = sqrt(pow((pose.x - local_path[segment+1][1]),2) + pow((pose.y - local_path[segment+1][2]),2));
	 L2 = sqrt(pow((local_path[segment+1][1] - local_path[segment][1]),2) + pow((local_path[segment+1][2] - local_path[segment][2]),2));
	 L3 = sqrt(pow((pose.x - local_path[segment][1]),2) + pow((pose.y - local_path[segment][2]),2));
...
// return 0 if goal unreachable
	...
	for (int i = 0; i < local_path.size(); i++)
	{
		...
		while (along_track_error > error_tolerance2goal) 
		{
			...
			cout << "bendahead-----"<< bendahead << "pose.heading-----"<< pose.heading << endl;
			...
			{
				segment_length = sqrt(pow((local_path[segment+1][1] - local_path[segment][1]),2) + pow((local_path[segment+1][2]- local_path[segment][2]),2));	
