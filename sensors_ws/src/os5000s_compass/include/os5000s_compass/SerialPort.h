...
#define SERIALPORT_H
/************************
  SeiralPort.h and SerialPort.cpp give a wrapper for simple serial IO  compile using libudev (-ludev).

  To open a serial port you can use
    SerialPort myPort("/dev/mySerialTTY/",B_9600); // Where B_9600 specifies the baud rate.
    or
    SerialPort myPort;
    myPort.openPort("/dev/mySerialTTY/",B_9600);
  ...
  To find out USB_Path using vendorID and productID:
  getDevPath(string vendorID, string productID) //returns a vector of USB_Paths as strings
...
public:
    ...
    bool writeData( const char* buffer, int bytes );
    bool writeByte( char byteToWrite );
    bool writeString( const std::string& data );

    unsigned char readByte(double ms_timout = 0 );

    int readData( char *buffer, int bytes, double ms_timout = 0, char startDelim = 0, char endDelim = 0 );
    std::string getLine( char endDelim = '\n' );
    ...
    static vector<SerialPort::DeviceInfo> getDevPath(string vendorID, string productID);
};
