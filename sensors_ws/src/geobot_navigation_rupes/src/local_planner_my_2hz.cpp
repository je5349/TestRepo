...
//*************************************************************************************//
	...
	//minimum_velocity_constraint = 0.1;
...
bool LocalPlannerUtil::path_tracking_controller(double total_distance, int mode)
{
	...
	/*if(left < right){
		local_path.push_back(pose.y - ((left+right)/6));
	}
	if(right < left){
		local_path.push_back(pose.y + ((left+right)/6));
	}*/
	...
	if(left < right){
		local_path.push_back(pose.y - ((right-left)/6)); // was /2
	}
	if(right < left){
		local_path.push_back(pose.y + ((left-right)/6)); // was /2
	}
	...
	//ROS_INFO("Which side of the path is geobot on: %d", onwhich_sideofrobot_is_originalpath);
	...
	choose_path();
			...
			if(path2select == OFFSET_ORIGINAL_PATH)
			{
				...
				//pose.x = pose.x + path_offset*cos(offset_direction);
				//pose.y = pose.y + path_offset*sin(offset_direction);
	...
	//else linear_velocity = 0;
			...
			/*if (error_heading < -0.4)
			{	
				error_heading = -0.4;
			}
			else if (error_heading > 0.4)
			{	
				error_heading = 0.4;
			}*/
...
//ROS_INFO("error difference component  %f", delta_cross_track_error);
			...
			//ROS_INFO("Controller Terms: %f", controller_error_term + controller_heading_error_term);
			//ROS_INFO("Direction: %f", direction);
			//ROS_INFO("Pose Heading: %f", pose.heading);
			ROS_INFO("Controller Error: %f", controller_error_term);
			ROS_INFO("Controller Error Heading: %f", controller_heading_error_term);
			// ROS_INFO("cte_cross_track_error: %f", atan2(cte_gain*cross_track_error,linear_velocity));
	...
	//{
		...
		//angular_velocity = angular_velocity_constraint*10*angular_velocity; 

		// if(mode == 1)
		// {
		// 	angular_velocity = -0.45*angular_velocity;
		// }
...
void LocalPlannerUtil::choose_path()
{
	...
	//ROS_INFO("closest_localpath_trajectory_heading = %f",closest_localpath_trajectory_heading);
	...
	int obstacle_array[3] = {LEFT_SIDE_OPENED, FRONT_SIDE_OPENED,  RIGHT_SIDE_OPENED}; // L F R
		...
		//ROS_INFO("LEFT_BLOCK: %d", obstacle_array[0]); 
		//ROS_INFO("FRONT_BLOCK: %d", obstacle_array[1]); 
		//ROS_INFO("RIGHT_BLOCK: %d", obstacle_array[2]); 
