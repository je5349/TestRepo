...
//*************************************************************************************//
    ...
    //compassSubscriber_ = nh.subscribe("compass_heading",10, &LocalPlannerUtil::get_compass,this);
...
bool LocalPlannerUtil::run(double a, double b)
{
	...
	if(!acquire_robotpose()) // update the robot pose
	{
		...
		//goal_reached = path_tracking_controller(d.dist[i],d.heading[i],0);
	...
	//call path tracking controller
	//returned = path_tracking_controller(d.dist[i-1],d.heading[i-1],1);
...
// Callback for joystick override over false obstacle.
void LocalPlannerUtil::joystickoveride(const sensor_msgs::Joy joy_msg)
{
    ...
    if (joy_msg.buttons[2] == 1)  // little red button for ctrl-c functionality on the logitech f710 joystick controller.
    {
        ros::shutdown();
    }
    else
...
bool LocalPlannerUtil::path_tracking_controller(double total_distance, int mode)
{
	...
	double compass_correction_gain = 0;
	//*************************************************************//
	...
	while (distance_travelled < total_distance  && ros::ok())
	{
		...
		if (mode == 1 && turn_around == false)
		{
			...
			{
				if (left > 0.7 && right > 0.7 && front > 0.7)
				{
				...
				{
					ROS_INFO("No space for turnaround");
				}
	...
	else
	{
		if(left < right)
		{
			local_path.push_back(pose.y - ((3-left)/4)); 
		}
		else if(right < left)
		{
			local_path.push_back(pose.y + ((3-right)/4)); 
		}
		else
		{
			local_path.push_back(pose.y);
	...
	choose_path();
		...
		else
		{
			...
			//compass_correction_gain = 0.1*(pose.heading - adit_direction);
			//compass_correction_gain = 0;
		}
		...
		//ROS_INFO("Compass Heading Correction: %f", compass_correction_gain);
...
//void LocalPlannerUtil::get_compass(const os5000s_compass::compass_heading msg)
//{
//	compass_msg = msg;
//}
...
int main(int argc, char **argv)
{
 ...
 if (argc != 3)
  ...
  /*ifstream file1;
  file1.open("/home/rupak/ros_ws/src/geobot_navigation_rupes/src/file.txt");
  if(!file1)
  {
      ROS_INFO("Cant open the heading heuristic file");
  }
  else    
  {
      while(!file1.eof())
      {
          float a1,b1;
	  	  ...
	  	  file1>>b1;
          d.dist.push_back(a1);
          d.heading.push_back(b1);
      }
      ROS_INFO("Reached EOF");
   }
   no_of_waypoints = d.dist.size();*/
